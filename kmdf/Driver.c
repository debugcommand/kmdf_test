...
//        FILE_WRITE_DATA)
//
//#define KMDF_PARAM_QUEUE_SIZE_DEFAULT      1048576     /* 1MB */
//#define KMDF_PARAM_QUEUE_LENGTH_DEFAULT    1024
//
//#pragma pack(push, 1)
//typedef union
//{
...
//	ULONGLONG data_queue_maxlength;           // Packet queue max length.
//	ULONGLONG data_queue_size;                // Packet queue size (in bytes).
//	ULONGLONG data_queue_maxsize;             // Packet queue max size.
...
//	BOOLEAN isCreate;
//};
//typedef struct CUSTOMERDATA *PCUSTOMERDATA;
//static UINT32 data_size = sizeof(LIST_ENTRY) + sizeof(INT32) * 2 + sizeof(BOOLEAN);
///*
// * request context.
// */
...
//EVT_WDF_WORKITEM kmdf_worker;
//
//extern VOID kmdf_worker(IN WDFWORKITEM item);
//
//static POOL_TYPE non_paged_pool = NonPagedPool;
//#define KMDF_TAG                           'viDW'
///*
// * malloc/free.
// */
//static PVOID kmdf_malloc(SIZE_T size, BOOLEAN paged)
//{
...
//		return NULL;
//	}
//	return ExAllocatePoolWithTag(pool, size, KMDF_TAG);
//}
//static VOID kmdf_free(PVOID ptr)
//{
//	if (ptr != NULL)
//	{
//		ExFreePoolWithTag(ptr, KMDF_TAG);
//	}
//}
//
...
//		context->data_queue_length = 0;
//		context->data_queue_maxlength = KMDF_PARAM_QUEUE_LENGTH_DEFAULT;
//		context->data_queue_size = 0;
//		context->data_queue_maxsize = KMDF_PARAM_QUEUE_SIZE_DEFAULT;
//
//		KeInitializeSpinLock(&context->lock);
//
//		InitializeListHead(&context->work_queue);
//		InitializeListHead(&context->data_queue);
//
...
//		}
//
//		WDF_WORKITEM_CONFIG_INIT(&item_config, kmdf_worker);
...
//	KdPrint(("[MyWDFDevice]kmdf_caller_context 8"));
//	return;
...
// * Queue a data.
// */
//static void kmdf_queue_data(PDEVICE_CONTEXT context, PCUSTOMERDATA data)
//{
//	KLOCK_QUEUE_HANDLE lock_handle;
//	PLIST_ENTRY old_entry;
//	PCUSTOMERDATA old_packet;
//	LONGLONG timestamp;
//
...
//	while (TRUE)
//	{
//		if (context->state != CONTEXT_STATE_OPEN)
//		{
//			KeReleaseInStackQueuedSpinLock(&lock_handle);
//			kmdf_free_data(data);
//			return;
//		}
//		if (context->data_queue_size + data_size >
//			context->data_queue_maxsize ||
//			context->data_queue_length + 1 > context->data_queue_maxlength)
//		{
//			// The queue is full; drop a packet & try again:
//			old_entry = RemoveHeadList(&context->data_queue);
//			old_packet = CONTAINING_RECORD(old_entry, struct CUSTOMERDATA, entry);
//			context->data_queue_length--;
//			context->data_queue_size -= data_size;
//			KeReleaseInStackQueuedSpinLock(&lock_handle);
...
//		{
//			// Queue the packet:
//			InsertTailList(&context->data_queue, &data->entry);
//			context->data_queue_length++;
//			context->data_queue_size += data_size;
//			break;
//		}
...
//	KeReleaseInStackQueuedSpinLock(&lock_handle);
//
//	DEBUG("DATA: queued data (data=%p)", data);
//
//	return;
//}
//
///*
// * work item routine for out-of-band filtering.
// */
//VOID kmdf_worker(IN WDFWORKITEM item)
//{
//	KLOCK_QUEUE_HANDLE lock_handle;
//	WDFFILEOBJECT object = (WDFFILEOBJECT)WdfWorkItemGetParentObject(item);
//	PDEVICE_CONTEXT context = windivert_context_get(object);
//	PLIST_ENTRY entry;
//	PCUSTOMERDATA work;
//
//	KeAcquireInStackQueuedSpinLock(&context->lock, &lock_handle);
//	while (context->state == CONTEXT_STATE_OPEN &&
...
//		KeReleaseInStackQueuedSpinLock(&lock_handle);
//
//		work = CONTAINING_RECORD(entry, struct CUSTOMERDATA, entry);
//		kmdf_queue_packet(context, work);
//		KeAcquireInStackQueuedSpinLock(&context->lock, &lock_handle);
//	}
//	KeReleaseInStackQueuedSpinLock(&lock_handle);
//
...
// * Free a data.
// */
//static void kmdf_free_data(PCUSTOMERDATA packet)
//{
//	kmdf_free(packet);
//}